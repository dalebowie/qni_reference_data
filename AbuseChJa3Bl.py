...
class AbuseChJa3Bl():
    ...
    def download_full_database(self):
        ...
        resp = requests.get('https://sslbl.abuse.ch/blacklist/ja3_fingerprints.csv')
    ...
    def load_file_contents(self):
        ...
        with open(self.FILE_NAME, "r") as inFile:
            ...
            for content in contents:
                if len(content) != 4 or content[0][0] == '#':
                    # All rows should have 4 columns and should not begin with a comment symbol
                    # Skip over this row if it matches
                    continue
                
                self.hashes[content[self.COL_HASH]] = f"{content[self.COL_REASON]} (first seen on {content[self.COL_FIRST]}, last seen on {content[self.COL_LAST]})"
